...
public class ComputerGAME {
	...
	public void puckMove(){
		...
		if(userPaddleCollision()){ //if the user paddle hits the puck
			...
			if(userPaddleSpeed < 5){ //if the user paddle speed is less than 5, add it to the current puck speed
				puckSpeed += userPaddleSpeed;
			}else if(userPaddleSpeed <= 15){ //otherwise, assign it as the puck speed
				puckSpeed = userPaddleSpeed;
			...
			//transfers paddle speed to the puck
			if(opponentPaddleSpeed < 5){ //if the computer paddle speed is less than 5, add it to the current puck speed
				puckSpeed += opponentPaddleSpeed;
			}else if(userPaddleSpeed <= 15){ //otherwise, assign it as the puck speed
				puckSpeed = opponentPaddleSpeed;
			}else{ //ensures the max puck speed is 15;
				puckSpeed = 15;
			}
			puckStep(); //calls the puck step method so that the puck does not get stuck in the paddle
		}
	...
	private class ML implements MouseMotionListener{
		...
		public void mouseEvent(MouseEvent e){
			...
			if((mouseX - userPaddle.getRadius()) >= 50 && (mouseX + userPaddle.getRadius()) <= 394 && (mouseY - userPaddle.getDiameter()) >= 50 && ((mouseY) >= 322 && (mouseY)<=522)){ //if its inside the bounds
